/*
 * The text a PDF already contains.
 *
 * ── Where this came from ────────────────────────────────────────────────────
 *
 * This file is copied, not imported. It was written for a sibling project — a
 * document reading service — where the argument was that most PDFs already
 * carry their text and recognising it from the pixels instead is slower, costs
 * money per page, and gives a worse answer than the one that was already
 * there.
 *
 * It is here so somebody can drop a real PDF into this console and ask
 * questions about it, rather than being limited to the three invented manuals.
 * Copied rather than depended on because these are two independent
 * repositories: a shared package between them would be a third thing to
 * publish, version and keep in step, for six hundred lines that change about
 * once a year.
 *
 * What it does NOT do is read a scan. A PDF that is a photograph of a page
 * contains no text to find, and this says so rather than returning nothing —
 * see the sibling project for the half that reads pixels, which needs a key.
 */
/**
